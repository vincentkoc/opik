...
class HelpersMixin:
    def _get_task_description_for_llm(self, prompt: chat_prompt.ChatPrompt) -> str:
        ...
        description += (
            f"The original high-level instruction being optimized is: '{prompt.get_messages()}'. "
        )
        description += (
            "The goal is to create an effective prompt that guides a language model to perform this task well."
        )
        return description
