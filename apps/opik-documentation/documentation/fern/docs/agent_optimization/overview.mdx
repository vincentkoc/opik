...
description: "Learn about Opik's automated LLM prompt and agent optimization SDK. Discover MetaPrompt, Few-shot Bayesian, GEPA, MIPRO, and Evolutionary optimization algorithms for enhanced performance."
keywords: "LLM optimization, prompt engineering, agent optimization, MetaPrompt, Few-shot learning, Bayesian optimization, GEPA, MIPRO, Evolutionary algorithms, DSPy"
...
| [GEPA Optimization](/agent_optimization/algorithms/gepa_optimizer)                          | System optimization via reflective text evolution (Genetic-Pareto). Optimizes text components (e.g., system prompts) against metrics with reflective mutation and Pareto selection. Good for general prompt/code text. |
